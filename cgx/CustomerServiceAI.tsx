...
import { ChatIcon, CloseIcon, ArrowForwardIcon, RepeatIcon } from "@chakra-ui/icons";
...
4. 如果用户询问订单信息，你应该告诉用户你正在查询他们的订单
5. 回答要简洁、准确、专业，用友好的方式展示查询结果
6. 如果用户需要更复杂的帮助，建议他们联系真人客服
...
export function CustomerServiceAI() {
  ...
  useEffect(() => {
    if (isOpen && messages.length === 0) {
      ...
      setMessages([
        {
          role: "assistant",
          content: "您好！我是 ZJC 航空公司的 AI 客服助手。我可以帮助您查询订单、航班信息，或回答一些常见问题。如有需要，也可以帮您转接真人客服。请告诉我您需要什么帮助？",
          timestamp: new Date(),
        },
      ]);
    }
  ...
  // 按订单号查询订单
  const searchOrderByNumber = async (orderNo: string): Promise<BookingItem | null> => {
    try {
  ...
  const searchOrders = async (query?: string): Promise<BookingItem[]> => {
    try {
      ...
      const userId = typeof auth.id === "number" 
        ? auth.id 
        : parseInt(String(auth.id), 10);
  ...
  const parseDateQuery = (query: string): string | null => {
    ...
    // 检查具体日期格式：YYYY-MM-DD 或 YYYY/MM/DD
  ...
  // 按航班号查询航班
  const searchFlightByNumber = async (flightNumber: string): Promise<Flight | null> => {
    try {
  ...
  const searchFlights = async (query?: string): Promise<Flight[]> => {
    try {
      ...
      // 如果没有匹配到，尝试更简单的模式：北京-上海 或 北京到上海
      if (!from && !to) {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const shouldQueryFlights =
        ...
        userMessage.match(/\d{4}[-\/]\d{1,2}[-\/]\d{1,2}/) || // 日期格式
        userMessage.match(/\d{1,2}月\d{1,2}日/); // 中文日期
      ...
      if (shouldQueryFlights) {
        ...
        if (foundFlights.length > 0) {
          ...
          displayFlights.forEach((flight, index) => {
            ...
            contextData += `   出发时间：${new Date(flight.departTime).toLocaleString('zh-CN')}\n`;
            contextData += `   到达时间：${new Date(flight.arrivalTime).toLocaleString('zh-CN')}\n`;
          ...
          if (foundFlights.length > 10) {
            contextData += `\n注：还有 ${foundFlights.length - 10} 个航班未显示，请缩小查询范围获取更精确的结果。\n`;
          }
      ...
      // 提取链接和快捷操作
      const links: ChatBubble["links"] = [];
      const quickActions: ChatBubble["quickActions"] = [];
      ...
      if (foundFlights.length > 0) {
        if (foundFlights.length === 1) {
          // 只有一个航班时，直接跳转到航班页面并带上查询参数
          const flight = foundFlights[0];
          quickActions.push({
            label: `查看航班 ${flight.flightNumber}`,
            action: () => {
              router.push(`/flights?from=${encodeURIComponent(flight.from)}&to=${encodeURIComponent(flight.to)}`);
              onClose();
            },
          });
        } else {
      ...
      const errorMessage: ChatBubble = {
        role: "assistant",
        content: "抱歉，我暂时无法处理您的请求。请稍后再试，或直接联系我们的真人客服：400-800-1234，或访问联系客服页面。",
        timestamp: new Date(),
      };
  ...
  useEffect(() => {
    ...
    const handleMouseUp = (e?: MouseEvent) => {
      ...
      if (moved) {
        ...
        const centerThresholdX = windowWidth * 0.25; // 中间50%区域
        const centerThresholdY = windowHeight * 0.25; // 中间50%区域
        ...
        if (inCenterX && inCenterY) {
          // 在中间区域，移动到最近的侧边
          const targetX = currentCenterX < centerX 
            ? 20  // 左侧
            : windowWidth - buttonWidth - 20;  // 右侧
          const targetY = Math.max(20, Math.min(windowHeight - buttonHeight - 20, currentPos.y));
  ...
  }, [isDragging, dragStart, dragOffset, windowWidth, windowHeight]);
      ...
      {!isOpen && windowWidth > 0 && windowHeight > 0 && (
        ...
        >
          ...
          {showTooltip && (
            <Box
              position="absolute"
              left={position.x < windowWidth / 2 ? "130px" : "auto"}
              right={position.x >= windowWidth / 2 ? "130px" : "auto"}
              top="50%"
      ...
      >
        ...
        <DrawerContent>
          ...
          <DrawerHeader borderBottomWidth="1px">
            <HStack spacing={3}>
              <Box position="relative" width="40px" height="40px" borderRadius="full" overflow="hidden">
                <Image
                  src="/IP源文件三视图1.png"
                  alt="AI 客服"
                  fill
          ...
          <DrawerBody p={0} display="flex" flexDirection="column">
            ...
            >
              {messages.map((msg, idx) => (
                <VStack key={idx} align={msg.role === "user" ? "flex-end" : "flex-start"} spacing={2}>
                  <Flex
                  ...
                  >
                    {msg.role === "assistant" && (
                      <Box position="relative" width="32px" height="32px" borderRadius="full" overflow="hidden" flexShrink={0}>
                        <Image
                          src="/IP源文件三视图1.png"
                          alt="AI 客服"
                          fill
